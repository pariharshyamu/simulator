...
pres.author = 'MAHAGENCO Training Centre, Nashik';
...
L['chart-text'] = (s, t) => {
  ...
  s.addChart(pres.ChartType.line, t.chart.series.map(se => ({ name: se.name, labels: t.chart.categories, values: se.values })), {
    ...
    valAxisMinVal: 60, valAxisMaxVal: 95, valGridLine: { color: 'E4EAF0', size: 1 }, catGridLine: { style: 'none' },
    valAxisTitle: 'Bearing temperature (°C)', showValAxisTitle: true, valAxisTitleFontSize: 11, valAxisTitleColor: MUTED
  });
...
L.close = (s, t) => {
  ...
  s.addText('Questions — including from Koradi, Khaperkheda, Bhusawal and Paras', { x: 1.05, y: 6.78, w: 11.3, h: 0.4, fontFace: BODY, fontSize: 12, color: '8C9BA8', margin: 0 });
};
...
pres.writeFile({ fileName: 'AI_in_Power_Plants_MAHAGENCO_Deck.pptx' }).then(f => console.log('written', f));
