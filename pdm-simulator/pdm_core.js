...
/* =========================================================================
   1. EQUIPMENT CASES
   ========================================================================= */
const HOURS_PER_DAY = 24, DAYS = 200, N = DAYS*HOURS_PER_DAY;
...
function context(seed){
  ...
  for(let i=0;i<N;i++){
    ...
    load += (0.80+night+weekend-load)*0.22 + gauss(r)*0.020;
    load = clamp(load, 0.45, 1.0);
    const amb = 31 + 6.5*Math.sin((day/60)*Math.PI) - 5.5*Math.cos(hod/24*2*Math.PI) + gauss(r)*0.9;
    L.push(load); A.push(amb); CW.push(amb - 2.2 + gauss(r)*0.5);
  }
...
idfan:{
  ...
  gen(ctx, f, r, opt){
    ...
    for(let i=0;i<N;i++){
      ...
      d.oilT[i] = 42 + 0.38*(A-31) + 3.2*L + 9.5*F + gauss(r)*0.45;
...
xfmr:{
  ...
  gen(ctx, f, r, opt){
    ...
    for(let i=0;i<N;i++){
      ...
      d.topOil[i] = A + 22*Math.pow(L,1.8) + 24*F + gauss(r)*0.5*opt.noise;
      d.wti[i]    = d.topOil[i] + 13*Math.pow(L,1.6) + 10*F + gauss(r)*0.55*opt.noise;
...
/* healthyScore: the same model run on a sister machine that never develops a fault.
   That is where the nuisance-alert rate actually comes from — 200 days of it, not seven. */
function evaluate(score, faultDay, trainEnd, thr, persistH, healthyScore){
  ...
  let fa, hours;
  if(healthyScore){ fa=countAlerts(healthyScore, trainEnd, T, thr, persistH); hours=T-trainEnd; }
...
function sweepThresholds(score, faultDay, trainEnd, persistH, alarmDay, healthyScore){
  ...
  for(const ph of PERS){
    for(let k=0.25;k<=12.01;k+=0.25){
      ...
      const e=evaluate(score, faultDay, trainEnd, thr, ph, healthyScore);
